...
describe('floor map context', () => {
  ...
  it('retains base points and cross-floor teleports for display without expanding route scope', () => {
    ...
    expect(store.navigationPointCandidates).toEqual([])
    store.selectPointCandidate('base-echo')
    expect(store.selectedEchoLocation?.id).toBe('base-echo')
    expect(store.pointCandidates).toHaveLength(2)
    expect(store.navigationPointCandidates).toEqual([])
    store.returnToPointCandidates()
    expect(store.selectedEchoLocation).toBeNull()
    expect(store.pointCandidates).toHaveLength(2)
    store.selectPointCandidates(['base-echo', 'other-travel'])
    expect(store.pointCandidates.map(({ id }) => id)).toEqual(['base-echo'])
    expect(store.navigationPointCandidates.map(({ id }) => id)).toEqual(['other-travel'])
    store.selectPointCandidate('other-travel')
    expect(store.selectedNavigationPoint?.id).toBe('other-travel')
    store.returnToPointCandidates()
    expect(store.selectedNavigationPoint).toBeNull()
    store.selectPointCandidates(['base-echo', 'missing-point'])
    expect(store.selectedEchoLocation?.id).toBe('base-echo')
    expect(store.pointCandidates).toEqual([])
