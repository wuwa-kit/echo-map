...
同日已接入阿维纽林重力切换：同步脚本读取全部地图的 `gravity.json`，保留 903 的 36 张反重力瓦片及点位 `gravityType`，并将瓦片加入资产库。普通/反重力共用地图坐标网格，切换保留视口；URL 使用 `gravity=2` 保存反重力模式，普通模式省略参数。点位显示、筛选数量和路线候选同步切换，切换时取消旧路线；官方点的 Z=0 占位规则继续保留。

本次同步中，阿维纽林业务点位为普通重力声骸 266 个、普通重力定位点 18 个、反重力定位点 3 个。反重力声骸尚无官方标注，界面提示待核验，不据此推断游戏中不存在声骸。旧人工点的缺失重力字段读取为 `null`，仅在普通模式显示，核验前不参与该地图路线；录入器支持选择重力模式，点位合并与附近匹配均区分重力。更新数据使用 `pnpm data:sync:map`、`pnpm data:convert-official`，随后运行 `pnpm data:validate`。本段覆盖上文反重力相关待处理项，其余遗漏和来源差异仍待处理。
