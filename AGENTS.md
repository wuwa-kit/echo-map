...
- BOSS 仅作为可传送定位点显示，不作为声骸目标；BOSS 和官方副本挑战统一标记为可传送。人工定位点需核验 XYZ；官方导入点允许以 Z=0 测试路线。
- 定位点不提供手动显示筛选；同一图标内容可对应多个官方类型，但每个真实点位的传送模式、缩放等级和坐标数据保持独立。
