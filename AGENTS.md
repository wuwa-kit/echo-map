...
- 游戏内人工录入的整数 XYZ 是路线规划的权威坐标。官方地图坐标只能作为临时 XY 展示数据，不推断或伪造 Z。
...
- BOSS 仅作为可传送定位点显示，不作为声骸目标；BOSS 和官方副本挑战统一标记为可传送，但仍需人工补齐可靠 XYZ 后才能作为路线起点。
- 定位点显示筛选按图标内容分组；同一图标组可包含多个官方类型，但每个真实点位的传送模式、缩放等级和坐标数据保持独立。
- 点位是否显示与是否可参与路线相互独立。只有确认可直接传送且已人工补齐可靠 XYZ 的定位点才能作为路线起点。
