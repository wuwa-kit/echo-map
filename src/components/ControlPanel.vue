...
<template>
  <div
    class="absolute inset-y-0 right-0 z-80 min-h-0 w-[var(--control-panel-width)] overflow-visible transition-transform duration-220 ease"
    :class="collapsed ? 'translate-x-full' : 'translate-x-0'"
    role="complementary"
    aria-label="地图数据面板"
  >
    <button
      class="absolute left-[-42px] top-14px z-2 grid h-42px w-42px cursor-pointer place-items-center border border-[rgba(169,207,192,0.2)] border-r-0 rounded-l-9px bg-[rgba(9,20,18,0.94)] text-[var(--accent)] font-inherit shadow-[-8px_8px_24px_rgba(0,0,0,0.2)] backdrop-blur-12px hover:bg-[rgba(21,55,45,0.96)] hover:text-[#d9fff2] focus-visible:bg-[rgba(21,55,45,0.96)] focus-visible:text-[#d9fff2] focus-visible:outline-none"
      type="button"
      :aria-label="collapsed ? '展开地图数据面板' : '收起地图数据面板'"
      :aria-expanded="!collapsed"
      aria-controls="control-panel-content"
      :title="collapsed ? '展开地图数据面板' : '收起地图数据面板'"
      @click="emit('toggle')"
    >
      <WuSvg
        name="chevron-right"
        class="[--wu-svg-h:18px] transition-transform duration-180 ease"
    ...
    <WuScrollArea
      ...
      class="h-full border-l border-[var(--line)] bg-[var(--panel)] shadow-[-18px_0_45px_rgba(0,0,0,0.18)]"
      :aria-hidden="collapsed"
      :inert="collapsed"
    >
      <div v-if="dataset" class="border-b border-[var(--line)] p-18px max-[900px]:p-14px" role="banner">
        <div class="flex items-center justify-between gap-12px">
          ...
          <span
            class="shrink-0 text-right text-8px text-[#71877e] leading-[1.45]"
        ...
        <div class="mt-14px grid grid-cols-3 gap-6px">
          <div class="min-w-0 border border-[var(--line)] rounded-6px bg-[rgba(24,43,37,0.56)] px-8px py-7px">
            ...
            <span class="mt-2px block text-8px text-[#71877e]">C1/C3 声骸</span>
          </div>
          <div class="min-w-0 border border-[var(--line)] rounded-6px bg-[rgba(24,43,37,0.56)] px-8px py-7px">
            <span class="block truncate text-14px text-[#d8eee5] font-600">{{ dataset.echoLocations.length.toLocaleString('zh-CN') }}</span>
            <span class="mt-2px block text-8px text-[#71877e]">声骸点</span>
          </div>
          <div class="min-w-0 border border-[var(--line)] rounded-6px bg-[rgba(24,43,37,0.56)] px-8px py-7px">
            <span class="block truncate text-14px text-[#d8eee5] font-600">{{ dataset.navigationPoints.length.toLocaleString('zh-CN') }}</span>
            <span class="mt-2px block text-8px text-[#71877e]">定位点</span>
          </div>
        </div>
      </div>

      <div class="border-b border-[var(--line)] p-18px max-[900px]:p-14px">
        <div class="mb-13px flex items-center justify-between gap-12px">
          <span class="text-8px text-[#608176] font-800 tracking-[0.18em]">MAP SCOPE</span>
          <span class="flex items-center text-10px text-[var(--accent)] font-600">
            <span class="mr-6px h-5px w-5px rounded-full bg-[var(--accent)] shadow-[0_0_8px_var(--accent)]" />官方数据快照
          </span>
        </div>
        <label class="mb-6px block text-10px text-[var(--muted)]" for="state-select">地图</label>
        <WuSelect
          id="state-select"
        ...
        <div class="mt-10px grid grid-cols-2 gap-8px">
          <div>
            <label class="mb-6px block text-10px text-[var(--muted)]" for="country-select">地区</label>
            <WuSelect
              id="country-select"
          ...
          <div>
            <label class="mb-6px block text-10px text-[var(--muted)]" for="level-select">楼层</label>
            <WuSelect
              id="level-select"
      ...
      <div class="border-b border-[var(--line)] p-18px max-[900px]:p-14px">
        <div class="mb-11px flex items-center justify-between gap-10px">
          <div>
            <span class="block text-8px text-[#608176] font-800 tracking-[0.18em]">MAP POINT ICONS</span>
            <div class="mt-4px text-14px text-[#e7f1ec] font-[650]">定位点显示</div>
          </div>
          <span class="text-9px text-[var(--accent)]">{{ visiblePointGroupCount }}/{{ pointGroupOptions.length }} 图标</span>
        </div>
        <div class="mb-9px flex items-center justify-end gap-5px">
          <button class="cursor-pointer border border-[var(--line)] rounded-5px bg-transparent px-7px py-4px text-9px text-[#9db1a9] font-inherit hover:border-[rgba(101,241,194,0.4)] hover:text-[var(--accent)]" type="button" @click="store.showAllPointGroups">全部显示</button>
          <button class="cursor-pointer border border-[var(--line)] rounded-5px bg-transparent px-7px py-4px text-9px text-[#9db1a9] font-inherit hover:border-[rgba(101,241,194,0.4)] hover:text-[var(--accent)]" type="button" @click="hideAllCurrentPointGroups">全部隐藏</button>
        </div>
        <div class="mb-8px text-8px text-[#6f877e] leading-[1.5]">勾选表示允许显示；地图缩小时仍会按点位优先级自动隐藏。</div>
        <WuScrollArea class="max-h-230px" content-class="flex flex-col gap-4px pr-2px">
          <WuCheckBox
            v-for="pointGroup in pointGroupOptions"
            :key="pointGroup.id"
            :model-value="!hiddenPointGroupSet.has(pointGroup.id)"
            :title="pointGroup.typeNames.join(' / ')"
            class="grid min-h-38px cursor-pointer grid-cols-[16px_28px_minmax(0,1fr)_auto] items-center gap-7px rounded-6px border border-transparent bg-[rgba(25,43,38,0.48)] px-7px py-4px hover:border-[rgba(101,241,194,0.28)] hover:bg-[rgba(34,65,55,0.56)]"
            @update:model-value="store.setPointGroupVisible(pointGroup.id, $event)"
          >
            <span class="grid h-26px w-26px place-items-center">
              <img v-if="pointGroup.iconUrl" class="h-24px w-24px object-contain" :class="pointModeOpacityClass(pointGroup.modes)" :src="pointGroup.iconUrl" alt="" />
              <span v-else class="h-6px w-6px rounded-full bg-[#91a69e]" :class="pointModeOpacityClass(pointGroup.modes)" />
            </span>
            <span class="min-w-0">
              <span class="block overflow-hidden text-ellipsis whitespace-nowrap text-10px text-[#d5e3dd]">{{ pointGroup.name }}</span>
              <span class="mt-2px block text-8px text-[#6f877e]">{{ pointModeLabel(pointGroup.modes) }} · {{ pointZoomLabel(pointGroup.minZooms) }} · {{ pointGroup.typeCount }} 类</span>
            </span>
            <span class="text-9px text-[#82988f]">{{ pointGroup.count }}</span>
          </WuCheckBox>
          <div v-if="pointGroupOptions.length === 0" class="py-12px text-center text-9px text-[#71877e]">当前地图没有定位点图标</div>
        </WuScrollArea>
      </div>

      <div class="flex flex-wrap gap-x-15px gap-y-8px border-b border-[var(--line)] px-18px py-13px max-[900px]:px-14px">
        <WuCheckBox
          class="flex items-center gap-5px text-10px text-[#a7b8b1]"
      ...
      <div class="border-b border-[var(--line)] p-18px max-[900px]:p-14px">
        <div class="mb-13px flex items-center justify-between gap-12px">
          <div>
            <span class="block text-8px text-[#608176] font-800 tracking-[0.18em]">SONATA EFFECT</span>
            <div class="mt-4px text-14px text-[#e7f1ec] font-[650]">按合鸣效果筛选</div>
          </div>
          <button class="cursor-pointer border-0 bg-transparent p-2px text-10px text-[#789087] font-inherit" type="button" @click="store.clearSonataFilters">清除</button>
        </div>
        <WuScrollArea
          class="max-h-148px"
          content-class="grid grid-cols-4 gap-6px pr-2px max-[900px]:grid-cols-3"
        >
          <button
            v-for="sonata in dataset?.sonatas"
            :key="sonata.id"
            class="flex h-64px min-w-0 cursor-pointer flex-col items-center rounded-6px border px-3px pb-4px pt-6px font-inherit"
          ...
          >
            <img v-if="sonata.iconUrl" class="h-36px w-36px object-contain" :src="sonata.iconUrl" alt="" />
            <span class="w-full overflow-hidden text-ellipsis whitespace-nowrap text-8px text-[#b9c9c2]">{{ sonata.name }}</span>
          </button>
        </WuScrollArea>
      </div>
      ...
      <div class="border-b border-[var(--line)] p-18px max-[900px]:p-14px">
        <div class="mb-13px flex items-center justify-between gap-12px">
          <div>
            <span class="block text-8px text-[#608176] font-800 tracking-[0.18em]">ECHO TARGETS</span>
            <div class="mt-4px text-14px text-[#e7f1ec] font-[650]">选择声骸</div>
          </div>
          <span class="text-10px text-[var(--accent)] font-600">{{ visibleEchoLocations.length }} 点</span>
        </div>
        <div class="mb-10px flex h-34px w-full items-center gap-7px border border-[var(--line)] rounded-7px bg-[rgba(21,40,35,0.78)] px-10px text-[#6f887f] focus-within:border-[rgba(101,241,194,0.55)]">
          <span>⌕</span>
          <input class="w-full border-0 bg-transparent text-11px text-[#dce9e3] font-inherit outline-none" :value="echoSearch" type="search" :placeholder="`搜索 ${dataset?.echoes.length ?? 0} 个 C1/C3 声骸`" @input="onEchoSearch" />
        </div>
        <WuScrollArea
          class="max-h-290px max-[680px]:max-h-220px"
        ...
        >
          ...
          >
            <img class="h-38px w-38px object-contain" :src="echo.iconUrl" alt="" />
            <span class="flex overflow-hidden flex-col items-start gap-3px">
              <span class="max-w-full overflow-hidden text-ellipsis whitespace-nowrap text-11px text-[#dce9e3] font-[560]">{{ echo.name }}</span>
              <span class="text-9px text-[#70877e]">COST {{ echo.cost }} · {{ locationCountByEcho.get(echo.id) ?? 0 }} 处</span>
            </span>
            <span class="text-center text-13px text-[var(--accent)]">{{ selectedEchoIds.includes(echo.id) ? '✓' : '+' }}</span>
          </button>
        </WuScrollArea>
        <button class="mt-10px w-full cursor-pointer border border-[var(--line)] rounded-6px bg-transparent p-8px text-10px text-[#8fa49c] font-inherit" type="button" @click="store.clearFilters">重置声骸与合鸣筛选</button>
      </div>
    </WuScrollArea>
  </div>
</template>
