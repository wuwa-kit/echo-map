...
function showHint(event: PointerEvent, text: string): void {
...
<template>
  <div
    ...
    :class="compactFloors ? 'w-56px rounded-bl-9px rounded-br-9px rounded-tl-9px' : 'w-176px rounded-9px'"
  >
    <div class="relative h-44px shrink-0">
      <button
        type="button"
        class="h-44px cursor-pointer border-0 rounded-tl-8px text-left text-inherit"
        :class="[
        ...
        @click="selectLevel(null)" @pointerenter="showHint($event, '主地图')" @pointerleave="hideHint"
      >
      ...
      <button
        ...
        class="absolute top-0 h-44px flex cursor-pointer items-center justify-center text-[#a9bfb2] hover:bg-[#183329] hover:text-[#8cf4ce]"
        :class="compactFloors ? 'left-full w-32px rounded-r-8px border border-l-0 border-[var(--line)] bg-[#07110fed]' : 'right-0 w-36px rounded-tr-8px border-0 border-l border-[var(--line)] bg-transparent'"
        @click="toggleLayout"
    ...
    <div class="min-h-0 overflow-y-auto overscroll-contain rounded-b-8px border-t border-[var(--line)] [scrollbar-color:#516459_transparent]" :class="compactFloors ? '[scrollbar-width:none]' : '[scrollbar-width:thin]'" @scroll="hideHint">
      <div v-for="group in nearbyFloorGroups" :key="group.id" class="border-t border-[var(--line)] first:border-t-0" :class="compactFloors ? 'py-4px' : ''">
        ...
        <button
          ...
          @click="selectLevel(floor.id)" @pointerenter="showHint($event, floorHint(group.name, floor.name, floor.id))" @pointerleave="hideHint"
        >
    ...
    <div v-if="compactFloors && hint" class="pointer-events-none fixed z-90 -translate-y-1/2 break-words rounded-r-4px bg-[#e5eee7] px-12px py-8px text-13px font-600 text-[#263b31] shadow-lg" :style="{ left: `${hint.left}px`, top: `${hint.top}px`, maxWidth: `min(240px, calc(100vw - ${hint.left + 12}px))` }">{{ hint.text }}</div>
  </div>
</template>
