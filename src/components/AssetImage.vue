...
defineProps<{ src: string; large?: boolean }>()
...
<template>
  <div class="relative flex items-center justify-center overflow-hidden bg-[#12221f]">
    ...
    <img v-if="!failed" :src="src" loading="lazy" decoding="async" referrerpolicy="no-referrer" class="relative h-full w-full object-contain" :class="large ? 'p-12px' : 'p-8px'" @load="loaded = true" @error="failed = true">
  </div>
</template>
